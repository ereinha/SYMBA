...
class PreferenceDataset(Dataset):
    def __init__(self,file_index,preference_pairs, target,decoder_tokenizer,max_length=80, padding_idx=0):
    ...
    def __getitem__(self, idx):
        ...
        self.target = torch.Tensor(self.pad_sequence(np.array(self.target.reshape(-1).to('cpu')).astype(np.int32))).long().to("cuda")

        return self.file_index,self.target,torch.Tensor(better_expr.astype(np.int32)).long(), torch.Tensor(worse_expr.astype(np.int32)).long()
